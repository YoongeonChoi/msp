...
`remote_feature_artifacts=1` when those post-publication byte checks were
actually run.
...
2. Keep the current Codex Security scan summary bound to the final source state:
   after any repository edit, regenerate an external
   `security_scan_summary.json` for scan
   `d4415550-5104-47e5-a896-32ca72005b89`
   with current `source_head`, `source_diff_sha256`,
   `threat_model_receipt=true`, `finding_discovery_receipt=true`,
   `worklist_rows=8`, `completion_receipts=8`, `candidate_findings=0`,
...
6. Assemble the retained hosted, provider, provider-gap source, feature snapshot,
   incident, local, system-order-scope, and independent security replay outputs with
   `python -m app.tools.collect_live_readiness_evidence_bundle --provider-gap-evidence ... --verify-remote-provider-gap-artifacts --feature-evidence ... --incident-channel-evidence ... --system-order-scope-evidence ... --output ...`,
   then verify the generated bundle with
   `python -m app.tools.verify_live_readiness_evidence_bundle --evidence ... --verify-remote-provider-artifacts --verify-remote-incident-evidence --verify-remote-system-order-scope-evidence --verify-remote-feature-artifacts`
   until it returns `FINAL=PASS live_readiness_evidence_bundle` with
   `feature_evidence=1`, `remote_provider_artifacts=1`,
   `remote_incident_evidence=1`, `remote_system_order_scope_evidence=1`, and
   `remote_feature_artifacts=1`, with every
   ...
   incident-channel, system-order-scope, security report, provider lifecycle, or
   provider-live feature artifact evidence; retained URI reuse is checked after hostname/default-HTTPS-port
...
7. Retire the remaining provider warning by either proving a broker-wide
   ...
   `--system-order-scope-evidence ...`, `--provider-gap-evidence ...`, and
   `--verify-remote-provider-gap-artifacts` to
