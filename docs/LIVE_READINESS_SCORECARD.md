...
| Security posture | 98 | RLS policies remain admin-scoped, no anon/public writes were added, secrets remain placeholder/test-only, and the worker-only broker boundary is preserved. This iteration only changes `TradingLoop` continuous-mode handling for `KnownFailClosedError`: it records a warning event and continues the next cycle, while `RUN_ONCE=true` still propagates fail-closed setup errors for operator smoke validation. It does not touch `RiskService`, `ExecutionService`, `BrokerPort`, UI broker APIs, Supabase grants, or live-order approval paths. Worker verification returned `545 passed`; focused loop/trading-cycle tests returned `25 passed`; `ruff` and `mypy` passed for the changed worker files. The prior retained Codex Security delta scan `paper_health_self_noise_20260630151326` remains valid historical evidence for its reviewed diff, but it is no longer current source-bound evidence after this repository edit: `verify_security_scan_evidence` correctly returns `FINAL=FAIL security_scan_evidence reason=security_scan_evidence.source_head_mismatch;security_scan_evidence.source_diff_sha256_mismatch`. | Regenerate a formal source-bound `security_diff_scan` for the current committed source state, retain the markdown report, update `source_head` and `source_diff_sha256`, then pass `verify_security_scan_evidence --repo-root` and post-publication `--verify-remote-report-uri`. Strict release readiness still also requires manual Render redeploy observation for the new commit through hosted Supabase heartbeat/provider-health rows and a final bundle PASS with real hosted/provider/provider-gap/incident/scope artifacts, including provider-gap source artifact remote byte proof, `remote_provider_artifacts=1`, `remote_incident_evidence=1`, and `remote_system_order_scope_evidence=1`. Hosted Supabase role/RPC verification is tracked under the Supabase control-plane category. |
...
2. Keep the current Codex Security scan summary bound to the final source state:
   ...
   `C:\Users\choey\.tmp\codex-security-scans\msp\paper_health_self_noise_20260630151326\security_scan_summary.json`
   with current `source_head`, `source_diff_sha256`,
   `threat_model_receipt=true`, `finding_discovery_receipt=true`,
   `worklist_rows=2`, `completion_receipts=2`, `candidate_findings=0`,
...
6. Assemble the retained hosted, provider, provider-gap source, incident,
   ...
   `python -m app.tools.collect_live_readiness_evidence_bundle --provider-gap-evidence ... --verify-remote-provider-gap-artifacts --incident-channel-evidence ... --system-order-scope-evidence ... --output ...`,
