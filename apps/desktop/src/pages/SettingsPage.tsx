...
export function SettingsPage() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel>
        <SectionTitle title="Supabase API 연결" />
        ...
        <KeyValue label="데이터 경계" value="api schema V1 RPC/read model 전용" />
        ...
        {role.data ? (
          <>
            ...
            <KeyValue
              label="V1 역할"
              value={
                <Pill tone={(role.data.roles?.length ?? 0) > 0 ? "safe" : "warning"}>
                  {role.data.roles?.join(", ") || "읽기 불가"}
                </Pill>
              }
            />
            {role.data.warning ? (
